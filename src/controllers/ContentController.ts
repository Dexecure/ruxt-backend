...
export class ContentController implements IController {
  ...
  constructor(
    @inject(TYPES.BigQueryCalculatorService)
    bigQueryCalculatorService: BigQueryCalculatorService,
  ...
  handleContentRequest: express.RequestHandler = async (req, res) => {
    ...
    if (
      requiredProperties.reduce(
        (acc, key) => acc + (req.body.hasOwnProperty(key) ? 1 : 0),
        0
      ...
      const documentID = Object.keys(requestObject).reduce(
        (acc, key) => acc + requestObject[key],
        ""
      );
