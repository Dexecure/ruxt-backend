...
export class UpdateController implements IController {
  ...
  constructor(
    @inject(TYPES.ElasticSearchClient) esClient: ElasticSearchClient,
    ...
    return fileStream
      ...
      .flatMap(rankObj => {
        return this.esClient
          .searchByPrimaryOrigin(rankObj.domain)
          .then(docs => ({
            docs,
  ...
  updateRankings: express.RequestHandler = (req, res) => {
    ...
    this.updateElasticDocuments(rawFileStream).subscribe(
      d => {},
      () => {},
      () => {
        this.processing = false;
