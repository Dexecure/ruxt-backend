...
export class ElasticSearchClient {
  ...
  constructor(
    @inject(TYPES.ElasticSearchConnection)
    esConnection: ElasticSearchConnection,
    @inject(TYPES.IOHalter) ioHalter: IOHalter
  ) {
    this.esConnection = esConnection;

    const ioPromise = promisify(
      this.esConnection.esClient.ping.bind(this.esConnection.esClient)
  ...
  private createContentIndex(): Promise<any> {
    return promisify(
      this.esConnection.esClient.indices.create.bind(this.esConnection.esClient)
    )({
      index: this.esIndex,
      body: {
        mappings: {
  ...
  public addOrigin(origin: string): Promise<any> {
    return promisify(
      this.esConnection.esClient.create.bind(this.esConnection.esClient)
    )({
      index: this.esOriginIndex,
  ...
  public setOriginRank(
    id: string,
    rank: number,
    body: { origin: string }
  ): Promise<any> {
    return promisify(
      this.esConnection.esClient.index.bind(this.esConnection.esClient)
    )({
      index: this.esOriginIndex,
      type: this.esOriginType,
      id: id,
  ...
  public searchExactOrigin(origin: string): Promise<boolean> {
    ...
    return promisify(
      this.esConnection.esClient.search.bind(this.esConnection.esClient)
    )({
      index: this.esOriginIndex,
      body: {
